...
equipment_delta 是装备切换的唯一模拟输入：`-1` 表示上一件，`0` 表示不切换，`1` 表示下一件。采集层若同一 Tick 同时检测到“上一件”和“下一件”，规范化为 `0`。装备切换不再重复写入 action_bits，避免两个字段发生矛盾。
...
帧同步迁移拆成八份独立实施计划。每份计划必须在自己的自动验证、Headless 导入检查和人工验收通过后，才允许把其产出当成后续计划的稳定接口。

| Plan | 文档 | 主要交付物 | 进入下一阶段的门槛 |
|---|---|---|---|
| 1 | `2026-08-08-frame-sync-01-local-core.md` | 固定数学、PRNG、稳定 ID、帧命令、本地帧缓冲、固定 Tick、InputTape、规范状态、Hash 和资源版本格式 | 最小世界直接输入与编解码输入连续 100,000 Tick 的逐 Tick Hash 完全相同；失败则停止后续迁移 |
| 2 | `2026-08-08-frame-sync-02-map-assets-flow-field.md` | 离线资源构建契约、地图格、整数碰撞、动态占用、空间哈希、原子 Flow Field | 已提交地图资源可重复生成相同 Hash；流场不读取运行时 NavMesh 或异步结果 |
| 3 | `2026-08-08-frame-sync-03-player-shared-screen.md` | 1～4 玩家整数移动、地图碰撞、TeamBoundsSystem、最小玩家表现和纯表现共享摄像机 | 单人和本地 2～4 人沙盒均只由帧命令驱动；分辨率和摄像机变化不改变模拟位置 |
| 4 | `2026-08-08-frame-sync-04-zombie-horde.md` | 僵尸 SoA、稳定 ID、目标选择、状态机、局部分离和 100～150 只基线 | 150 只 Headless 群体模拟持续运行且无容量增长、无 Godot 物理/导航判定依赖 |
| 5 | `2026-08-08-frame-sync-05-combat.md` | 整数武器配置、枪械、近战、伤害、死亡和稳定表现事件 | 手枪、步枪、匕首在双实例回放中命中、弹药、死亡和事件 ID 逐 Tick 一致 |
| 6 | `2026-08-08-frame-sync-06-world-game-loop.md` | 油桶、爆炸、放置、拾取、掉落、库存、波次、失败和重开 | 无表现节点参与时可完成从开局、战斗、掉落、失败到重开的完整 Headless 循环 |
| 7 | `2026-08-08-frame-sync-07-view-migration-performance.md` | SimulationViewBridge、视图池、表现限流、整局功能开关和 100+ 渲染优化 | 新路径可选且完整可玩；旧节点在新会话中不再结算玩法，但旧 DemoArena 仍保留为默认回退 |
| 8 | `2026-08-08-frame-sync-08-determinism-qualification.md` | 固定输入录像、长期及跨平台 Hash、不同渲染节奏验证、性能档位、默认切换和未来传输接缝 | 单人、2 人、4 人录像各 100,000 Tick 零分歧，最低目标设备达到预算后才把新路径设为默认 |

~~~mermaid
flowchart LR
    P1["Plan 1 本地帧同步基础"] --> P2["Plan 2 资源管线、地图格与流场"]
    P2 --> P3["Plan 3 玩家与共享屏幕"]
    P2 --> P4["Plan 4 僵尸群体"]
    P3 --> P4
    P4 --> P5["Plan 5 战斗模拟"]
    P3 --> P6["Plan 6 世界交互与玩法循环"]
    P5 --> P6
    P6 --> P7["Plan 7 表现切换与 100+ 优化"]
    P7 --> P8["Plan 8 确定性资格验证"]
~~~

Plan 2 先冻结静态地图格和整数碰撞接口，Plan 3 才据此验收玩家地图碰撞；Plan 2 的动态占用和 Flow Field 可以与 Plan 3 后半段并行，但 Plan 4 必须等两者都完成。Plan 8 的测试工具骨架从 Plan 1 开始逐步扩充，每个 Plan 都必须增加自己的双实例录像与 Hash 场景，不能把确定性验证推迟到最后。任何阶段未达到门槛时，保持旧 DemoArena 为默认可玩路径，不通过混用旧节点判定来补齐新模拟的缺口。
...
当前 DemoArena 在 Plan 8 完成最终资格验证前继续作为默认可玩路径。新模拟使用独立测试入口或会话根部的整局功能开关，禁止按玩家、武器或子系统混用新旧玩法判定。Plan 7 只把新路径建设成可选且完整可玩；Plan 8 通过后才切换默认路径，并保留旧场景作为一个稳定周期内的回退入口。旧玩法判定逻辑的最终删除另开清理任务，不与默认切换同时进行。
...
- macOS、Windows、Web/WASM 和至少一个移动 ARM64 导出物读取同一资格录像时，逐检查点 Hash 必须一致；完成前只能称为本地确定性，不得称为跨平台帧同步就绪。
...
- 同一资格 InputTape 在 macOS、Windows、Web/WASM 和至少一个移动 ARM64 导出物之间零 Hash 分歧。
